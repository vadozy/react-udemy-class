        ...
        <div className="label">
          ...
          <div className="field">
            {props.portfolios.length === 0 ? <div className="loader" style={{position: "absolute", margin: "10px 0 0 60px", fontSize: "4px"}}>Loading...</div> : null}
              <select value={props.portfolio.portfolio} onChange={props.portfolioChange} >
                {props.portfolios.map(v => (<option key={v.portfolio} value={v.portfolio}>{v.portfolio}</option>))}
              </select>
          </div>
      </div>
    </section>
